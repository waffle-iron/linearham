...
Smooshable VSmooshable1(std::string yaml_path,
  ...
  // that corresponds to the first match starting position with a germline state.
  int start_pos = std::max(V_relpos - V_left_flexbounds.first, 0);
  Eigen::MatrixXd germ_prob_row =
      vgerm_obj.GermlineProbMatrix(V_left_flexbounds, V_right_flexbounds,
                                   emission_indices, V_relpos).row(start_pos);

  // multiplying in the associated gene and padding probabilities
  germ_prob_row *= vgerm_obj.gene_prob();
  double npadding_prob = vgerm_obj.NPaddingProb(V_left_flexbounds, emission_indices,
                                                V_relpos, true);
...
void MultiplyLandingGermProbMat(Eigen::VectorXd landing,
                                ...
                                int relpos) {
  if (relpos <= left_flexbounds.second) {
    int num_landing_rows = left_flexbounds.second - std::max(relpos, left_flexbounds.first) + 1;
...
std::pair<Smooshable, Smooshable> DSmooshables1(std::string yaml_path,
  ...
  Eigen::MatrixXd xgerm_prob_matrix =
      dgerm_obj.GermlineProbMatrix(V_right_flexbounds, D_right_flexbounds,
...
std::pair<Smooshable, Smooshable> JSmooshables1(std::string yaml_path,
  ...
  int end_pos = std::min(J_right_flexbounds.second, J_relpos + jgerm_obj.length()) - J_right_flexbounds.first;
  Eigen::MatrixXd xgerm_prob_col =
      jgerm_obj.GermlineProbMatrix(D_right_flexbounds, J_right_flexbounds,
  ...
  double npadding_prob = jgerm_obj.NPaddingProb(J_right_flexbounds, emission_indices,
